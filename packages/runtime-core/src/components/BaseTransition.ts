...
// Transition本身不会渲染额外内容，只是通过插槽获取需要过渡的元素，并渲染需要过渡的元素
// Transition本质上就是为需要过渡的元素的vnode上添加transition hooks
const BaseTransitionImpl: ComponentOptions = {
  ...
  setup(props: BaseTransitionProps, { slots }: SetupContext) {
    ...
    return () => {
      ...
      // 在<transition><keep-alive/></transition>的情况下，我们需要比较存活子节点的类型。
...
// transition hooks 以vnode.transition的形式添加到vnode上
// 并将在渲染器中适当的时间被调用。
export function resolveTransitionHooks(
  ...
  const hooks: TransitionHooks<TransitionElement> = {
    ...
    // 离场，添加离场初态类和离场过渡类，在下一帧删除初态类，加上终态类
    leave(el, remove) {
      const key = String(vnode.key)
      if (el[enterCbKey]) {
...
// unmounted.
// 占位符实际上只处理一个特殊情况:KeepAlive
// 如果KeepAlive在leave阶段，我们需要返回一个KeepAlive
// 内容为空的占位符，以避免KeepAlive实例被
// 卸载
...
// 获取keepAlive包裹的vnode，以便后续为其添加transition钩子函数
